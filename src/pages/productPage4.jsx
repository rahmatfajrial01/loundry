...
import { FaArrowAltCircleRight, FaCartPlus, FaPauseCircle, FaTrash } from "react-icons/fa";
...
const PageProduct = () => {
    ...
    const orderValidator = useValidator({
        dibayar: {
            isValid: false,
            required: false,
            message: ''
    ...
    const onUpdateOrder = (status) => {
        ...
        axios.put(`${import.meta.env.VITE_BASE_URL}/orders/${order.id}/`, payload)
            .then(() => {
                alert("Sukses!!!")
            }).catch((error) => {
                alert(`Error ${error}`);
                                ...
                                <Row>
                                    <Col>
                                        ...
                                        {!orderValidator.validator.customerName.isValid && (
                                            <small className="text-danger">{orderValidator.validator.customerName.message}</small>
                                        )}
                                    </Col>
                                    <Col>
                                        <Form.Control
                                        ...
                                        {!orderValidator.validator.customerPhone.isValid && (
                                            <small className="text-danger">{orderValidator.validator.customerPhone.message}</small>
                                        )}
                                    </Col>
                                </Row>
                            </Card.Body>
                                ...
                                <tfoot>
                                        ...
                                        <td colSpan={2}>
                                            <Form.Group>
                                                ...
                                                {!orderValidator.validator.dibayar.isValid && (
                                                    <small className="text-danger">{orderValidator.validator.dibayar.message}</small>
                                                )}
                                            </Form.Group>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <th colSpan={2}>
                                            ...
                                            {/* {(!orderValidator.isAllValid()).toString()} */}
                                        </th>
                                    </tr>
                                    <tr>
                                        <td colSpan={5} >
                                            <div className="d-flex justify-content-between gap-2">
                                                {!order.id && (
                                                    <Button
                                                        disabled={!orderValidator.isAllValid()}
                                                        className="w-100"
                                                        variant="warning"
                                                        onClick={() => onCreateOrder(import.meta.env.VITE_STATUS_HOLD)}>
                                                ...
                                                {order.id && (
                                                    <Button
                                                        ...
                                                        className="w-100"
                                                        variant="warning"
                                                        onClick={() => onUpdateOrder(import.meta.env.VITE_STATUS_HOLD)}>
                                                ...
                                                {!order.id && (
                                                    <Button
                                                        ...
                                                        className="w-100"
                                                        variant="success"
                                                        onClick={() => onCreateOrder(import.meta.env.VITE_STATUS_FINISH)}>
                                                ...
                                                {order.id && (
                                                    <Button
                                                        ...
                                                        className="w-100"
                                                        variant="success"
                                                        onClick={() => onUpdateOrder(import.meta.env.VITE_STATUS_FINISH)}>
                                                        <FaArrowAltCircleRight /> Process
